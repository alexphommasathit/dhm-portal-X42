...
export type AuditLogEntry = Omit<Database['public']['Tables']['audit_logs']['Insert'], 'id' | 'created_at'>;
...
export class AuditLogger {
   ...
   */
  async logEvent(event: Omit<AuditLogEntry, 'timestamp' | 'ip_address' | 'user_agent'>): Promise<boolean> {
    try {
  ...
  async syncPendingLogs(): Promise<number> {
    try {
      ...
      const { error } = await this.supabase
        .from('audit_logs')
        .insert(pendingLogs.map((log: any) => {
