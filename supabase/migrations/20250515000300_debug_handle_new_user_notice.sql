-- Add more detailed logging to handle_new_user using RAISE NOTICE
...
BEGIN
  RAISE NOTICE '[handle_new_user] Trigger started for user_id: %, email: %', NEW.id, NEW.email;
  RAISE NOTICE '[handle_new_user] raw_app_meta_data: %', NEW.raw_app_meta_data;

  BEGIN
    profile_role := (NEW.raw_app_meta_data->>'role')::public.user_role;
    RAISE NOTICE '[handle_new_user] Extracted role: %', profile_role;
  EXCEPTION
  ...
  IF profile_role IS NULL THEN
    RAISE NOTICE '[handle_new_user] profile_role was NULL, defaulting to unassigned.';
    profile_role := 'unassigned';
  END IF;

  profile_first_name := NEW.raw_app_meta_data->>'first_name';
  IF profile_first_name IS NULL OR TRIM(profile_first_name) = '' THEN
    RAISE NOTICE '[handle_new_user] first_name was NULL or empty, defaulting.';
    profile_first_name := 'Invited';
  END IF;
  RAISE NOTICE '[handle_new_user] Extracted first_name: %', profile_first_name;

  profile_last_name := NEW.raw_app_meta_data->>'last_name';
  IF profile_last_name IS NULL OR TRIM(profile_last_name) = '' THEN
    RAISE NOTICE '[handle_new_user] last_name was NULL or empty, defaulting.';
    profile_last_name := 'User';
  END IF;
  RAISE NOTICE '[handle_new_user] Extracted last_name: %', profile_last_name;

  profile_job_title := NEW.raw_app_meta_data->>'job_title';
  RAISE NOTICE '[handle_new_user] Extracted job_title: %', profile_job_title;
  
  RAISE NOTICE '[handle_new_user] Attempting to insert profile with user_id: %, email: %, role: %', NEW.id, NEW.email, profile_role;
  ...
  RAISE NOTICE '[handle_new_user] Profile INSERT successful for user_id: %', NEW.id;
  RETURN NEW;
EXCEPTION
  WHEN OTHERS THEN
